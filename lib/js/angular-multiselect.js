...
angular.module('ui.multiselect', [])
    ...
    function ($parse, $document, $compile, $interpolate, optionParser) {
      return {
        ...
        link: function (originalScope, element, attrs, modelCtrl) {
		  ...
		  /******** Watchlist Specific Functionality START ***********/
		  scope.selecteditem = {value: -1}; //for managing editing of items in the list
		  ...
		  scope.editWatchlist = function(){
			...
			scope.isEditSelected = !scope.isEditSelected;
			scope.editlabel = scope.isEditSelected ? "Ok" : "Edit";
		  };
		  //Edit Watchlist item functionality
		  scope.renameItem = function(item){
			scope.isRenameItemSelected = !scope.isRenameItemSelected;
			scope.compareWatchlistName = scope.isRenameItemSelected ? item.label.name : scope.compareWatchlistName; 
						
			if (!scope.isRenameItemSelected && scope.compareWatchlistName != item.label.name){
				console.log("** Run rename service call here **");
				console.log("scope.compareWatchlistName: " + scope.compareWatchlistName + ", item: " + item.label.name);
			}
		  ...
		  /******** Watchlist Specific Functionality END ***********/
          ...
          scope.$watch(function () {
			...
			//this was preventing the user from typing more than 1 char at a time into the rename input.
			//only execute on first load, when there is no oldValue
            if (angular.isDefined(newVal) && !angular.isDefined(oldValue))
              parseModel();
          }, true);
  ...
  .directive('multiselectPopup', ['$document', function ($document) {
    return {
      ...
      scope: false,
      replace: true,
      templateUrl: '/src/templates/multiselect.tmpl.html',
